...
## Muon momentum storage

Muon's momentum buffer is held on CPU pinned memory in the
configured `precision.muon_momentum` storage dtype: `bf16` /
`fp16` / `fp32` (production default is `bf16`). The per-mb grad
sum accumulates directly in `s.mom_buf` (merged-accumulator
design); at `step()` time the buffer is cast to FP32,
orthogonalized via Newton-Schulz, applied to the param, and
recast to the storage dtype for the next cycle.
