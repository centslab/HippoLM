...
from src.models.ops._vendored.fla.layers.utils import get_layer_cache, get_unpad_data, index_first_axis, pad_input, update_layer_cache
from src.models.ops._vendored.fla.modules import FusedRMSNormGated, ShortConvolution
from src.models.ops._vendored.fla.ops.gdn2 import chunk_gdn2, fused_recurrent_gdn2

if TYPE_CHECKING:
    from transformers.processing_utils import Unpack

    from src.models.ops._vendored.fla.models.utils import Cache


class GatedDeltaNet2(nn.Module):
    r"""
    Gated DeltaNet 2 (GDN-2) layer.

    GDN-2 extends KDA's scalar-beta erase gate to two independent channel-wise
    gates: an erase gate ``b`` on the key axis and a write gate ``w`` on the
    value axis. The matrix-state recurrence is

        S_t = (I - k_t (b_t * k_t)^T) Diag(exp(g_t)) S_{t-1}
              + k_t (w_t * v_t)^T

    where ``b in R^K`` is the channel-wise erase gate, ``w in R^V`` is the
    channel-wise write gate, and ``g in R^K`` is the channel-wise log-decay
    inherited from KDA. Setting ``b = w = beta`` (scalar broadcast) recovers
    KDA exactly; further collapsing ``g`` to a scalar recovers Gated DeltaNet
    v1. Reference:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # GDN-2 channel-wise gates: b on the K axis, w on the V axis.
