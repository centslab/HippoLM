...
from src.models.ops._vendored.fla.ops.gdn2.chunk_bwd import chunk_gdn2_bwd
from src.models.ops._vendored.fla.ops.gdn2.chunk_fwd import chunk_gdn2_fwd
    ...
    **kwargs,
    ...
    Args:
        ...
        initial_state: optional ``[N, H, K, V]`` initial state in float32 (or
            ``[N, H, V, K]`` if ``transpose_state_layout=True``).
        ...
        transpose_state_layout: store the recurrent state in ``[V, K]`` layout instead
            of the default ``[K, V]``.
    ...
    if 'transpose_state_layout' in kwargs:
        if transpose_state_layout:
            raise ValueError("Cannot pass both `transpose_state_layout` and the deprecated `transpose_state_layout`.")
        warnings.warn(
            "`transpose_state_layout` is deprecated and renamed to `transpose_state_layout`.",
            DeprecationWarning,
            stacklevel=2,
        )
        transpose_state_layout = kwargs.pop('transpose_state_layout')
