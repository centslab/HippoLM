...
from src.models.ops._vendored.fla.ops.utils.softplus import softplus
from src.models.ops._vendored.fla.utils import input_guard
    ...
    **kwargs,
    ...
    Args:
        ...
        initial_state (Optional[torch.Tensor]): ``[N, HV, K, V]`` in float32
            (or ``[N, HV, V, K]`` if ``transpose_state_layout=True``).
        ...
        lower_bound (Optional[float]): when ``use_gate_in_kernel=True`` and set,
            ...
            activation.
        transpose_state_layout (bool): store state as ``[V, K]`` instead of the default
            ``[K, V]``.
