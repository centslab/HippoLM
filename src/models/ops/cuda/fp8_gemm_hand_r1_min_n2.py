"""R1-min-N2 wrapper (sanity check: R0 source but copied standalone + NUM_STAGES=2)."""
...
def _load() -> ctypes.CDLL:
    ...
    if _LIB is not None:
        return _LIB
    sm = _sm_tag()
    so_path = _LIB_DIR / f"fp8_hand_r1min_n2_{sm}.so"
    if not so_path.exists():
        raise RuntimeError(f"No R1min-n2 .so at {so_path}")
    lib = ctypes.CDLL(str(so_path), mode=ctypes.RTLD_LOCAL)
    lib.fp8_hand_r1min_n2_gemm_run.restype = ctypes.c_int
    lib.fp8_hand_r1min_n2_gemm_run.argtypes = [
...
def is_available() -> bool:
    if not torch.cuda.is_available():
        return False
    sm = _sm_tag()
    return (_LIB_DIR / f"fp8_hand_r1min_n2_{sm}.so").exists()


def fp8_gemm_hand_r1min_n2(A, B_nk, a_scale, b_scale, out=None):
    ...
    rc = _load().fp8_hand_r1min_n2_gemm_run(
        M, N, K,
    ...
    if rc != 0:
        raise RuntimeError(f"fp8_hand_r1min_n2_gemm_run failed (rc={rc})")
    return out