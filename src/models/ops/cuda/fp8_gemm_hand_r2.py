"""FP8 E4M3 GEMM via hand-written Round 2 kernel (sm_120).

Round 2 = Round 0 + 8 consumer warps in a 2x4 grid (each consumer
covers 64x32 of the 128x128 MmaTile). NUM_STAGES=2 kept from R0
(R1-min isolation showed NUM_STAGES=3 alone is BAD here: consumer
is faster than producer, deeper pipeline just adds initial-fill
latency without throughput gain).

Same per-MmaTile (1x1) FP32 scales as R0 / CUTLASS 87a.
...
def _load() -> ctypes.CDLL:
    ...
    if _LIB is not None:
        return _LIB
    sm = _sm_tag()
    so_path = _LIB_DIR / f"fp8_hand_r2_{sm}.so"
    if not so_path.exists():
        raise RuntimeError(
            f"No hand-written FP8 GEMM R2 for {sm} at {so_path}; run "
            "scripts/build_fp8_hand_gemm.py"
        )
    lib = ctypes.CDLL(str(so_path), mode=ctypes.RTLD_LOCAL)
    lib.fp8_hand_r2_gemm_run.restype = ctypes.c_int
    lib.fp8_hand_r2_gemm_run.argtypes = [
...
def is_available() -> bool:
    if not torch.cuda.is_available():
        return False
    sm = _sm_tag()
    return (_LIB_DIR / f"fp8_hand_r2_{sm}.so").exists()


def fp8_gemm_hand_r2(
    A: torch.Tensor,
    B_nk: torch.Tensor,
    ...
    """Per-MmaTile FP8 GEMM, R2 (8 consumer warps). See module docstring."""
    ...
    if rc != 0:
        raise RuntimeError(f"fp8_hand_r2_gemm_run failed (rc={rc})")
    return out