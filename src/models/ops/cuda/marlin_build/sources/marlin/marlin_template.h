...
#pragma once  // added for standalone build — the sm89 instantiations
              // file #include's this header; without a guard we'd
              // get duplicate `marlin::scale_float` etc.
...
template <typename scalar_t,  // compute dtype, half or nv_float16
          const marlin::vllm::ScalarTypeId b_type_id,  // weight MarlinScalarType id
          const marlin::vllm::ScalarTypeId s_type_id,  // weight scale ScalarType id
...
template <int count, marlin::vllm::ScalarTypeId type_id>
...
template <marlin::vllm::ScalarTypeId type_id>
...
template <const marlin::vllm::ScalarTypeId a_type_id,  // A ScalarType id
          const marlin::vllm::ScalarTypeId b_type_id,  // B ScalarType id
          const marlin::vllm::ScalarTypeId c_type_id,  // C ScalarType id
          const marlin::vllm::ScalarTypeId s_type_id,  // B_SCALE ScalarType id
          const int threads,          // number of threads in a threadblock
          const int thread_m_blocks,  // number of 16x16 blocks in the m
                                      // dimension (batchsize) of the
                                      // threadblock
...
__global__ void Marlin(
  ...
  if constexpr (a_type_id == marlin::vllm::kFE4M3fn.id()) return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  // Turing TensorCore only supports fp16 and int8
  if constexpr (a_type_id != marlin::vllm::kFloat16.id() && a_type_id != marlin::vllm::kS8.id())
    return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  constexpr auto num_bits = marlin::vllm::ScalarType::from_id(b_type_id).size_bits();
  // Disable use_fp16_accum for NVFP4 and cases when group_size == -1 &&
  // num_bits == 4
  constexpr bool use_fp16_accum =
      a_type_id == marlin::vllm::kFloat16.id() &&
      (!(b_type_id == marlin::vllm::kFE2M1f.id() && s_type_id == marlin::vllm::kFE4M3fn.id()) &&
  ...
  static constexpr auto a_type = marlin::vllm::ScalarType::from_id(a_type_id);
  static constexpr auto b_type = marlin::vllm::ScalarType::from_id(b_type_id);
  static constexpr auto c_type = marlin::vllm::ScalarType::from_id(c_type_id);
  static constexpr auto s_type = marlin::vllm::ScalarType::from_id(s_type_id);
  if constexpr (b_type == marlin::vllm::kFE2M1f) {
    static_assert(s_type == marlin::vllm::kFE4M3fn && group_blocks == 1 ||
                  s_type == marlin::vllm::kFE8M0fnu && group_blocks == 2);
  } else if constexpr (s_type == marlin::vllm::kFE8M0fnu) {
    // MXFP8: FP8 weights with e8m0 microscaling block scales
    static_assert(b_type == marlin::vllm::kFE4M3fn && group_blocks == 2);
  } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    static_assert(s_type == marlin::vllm::kBFloat16);
  } else if constexpr (std::is_same<scalar_t, half>::value) {
    static_assert(s_type == marlin::vllm::kFloat16);
  }
  ...
  constexpr bool has_zp = b_type == marlin::vllm::kU4 || b_type == marlin::vllm::kU8;
  constexpr bool is_int_type = b_type == marlin::vllm::kU4 || b_type == marlin::vllm::kU8 ||
                               b_type == marlin::vllm::kS4 || b_type == marlin::vllm::kS8 ||
                               b_type == marlin::vllm::kU4B8 || b_type == marlin::vllm::kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      is_a_8bit || (b_type == marlin::vllm::kFE4M3fn && !(s_type == marlin::vllm::kFE8M0fnu)) ||
      b_type == marlin::vllm::kFE2M1f && s_type == marlin::vllm::kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(b_type == marlin::vllm::kU8);

  float global_scale_f32 = 1.0f;

  if constexpr (b_type == marlin::vllm::kFE2M1f && s_type == marlin::vllm::kFE4M3fn) {
  ...
  auto matmul = [&](int k, int pipe) {
    ...
    if constexpr (s_type == marlin::vllm::kFE4M3fn || s_type == marlin::vllm::kFE8M0fnu) {
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (b_type_id == marlin::vllm::kFE2M1f.id()) {
  ...
  #pragma unroll
      ...
      if constexpr (group_blocks != -1) {
        if (group_blocks == 2 || k == 1) {
          if constexpr (a_type == marlin::vllm::kS8) {
            int2 s_vals[2];
            s_vals[0] = {
  ...
  #pragma unroll
            ...
            float2 s_vals[2];
            if constexpr (s_type_id != marlin::vllm::kFE8M0fnu.id()) {
