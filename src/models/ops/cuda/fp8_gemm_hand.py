"""FP8 E4M3 GEMM via hand-written Round 0 kernel (sm_120).
...
(see auto-memory `project_fp8_gemm_bottleneck.md`). Round 0 is a
minimal baseline: 128x128x128 MmaTile, 4 warps, NUM_STAGES=2, no warp
specialization. Built via ``scripts/build_fp8_hand_gemm.py``.
"""
...
def _load() -> ctypes.CDLL:
    ...
    if _LIB is not None:
        return _LIB
    sm = _sm_tag()
    so_path = _LIB_DIR / f"fp8_hand_r0_{sm}.so"
    if not so_path.exists():
        raise RuntimeError(
            f"No hand-written FP8 GEMM for {sm} at {so_path}; run "
            "scripts/build_fp8_hand_gemm.py"
        )
    lib = ctypes.CDLL(str(so_path), mode=ctypes.RTLD_LOCAL)
    lib.fp8_hand_r0_gemm_run.restype = ctypes.c_int
    lib.fp8_hand_r0_gemm_run.argtypes = [
...
def is_available() -> bool:
    if not torch.cuda.is_available():
        return False
    sm = _sm_tag()
    return (_LIB_DIR / f"fp8_hand_r0_{sm}.so").exists()


def fp8_gemm_hand_r0(
    A: torch.Tensor,
    B_nk: torch.Tensor,
    ...
    if rc != 0:
        raise RuntimeError(f"fp8_hand_r0_gemm_run failed (rc={rc})")
    return out