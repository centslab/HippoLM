"""FP8 E4M3 GEMM via hand-written Round 4 kernel (sm_120).

Round 4 = Round 2 + multi-lane TMA (lane 0 issues TMA_A, lane 1
issues TMA_B in parallel within the same producer warp). Same 8
consumer warps, same NUM_STAGES=2.

This breaks the single-lane TMA producer bottleneck that limited R2
to ~60-72 TF (67-80% of CUTLASS 87a). Expected gain: 10-20%.

Same per-MmaTile (1x1) FP32 scales as R0 / R2 / R3 / CUTLASS 87a.
...
def _load() -> ctypes.CDLL:
    ...
    if _LIB is not None:
        return _LIB
    sm = _sm_tag()
    so_path = _LIB_DIR / f"fp8_hand_r4_{sm}.so"
    if not so_path.exists():
        raise RuntimeError(
            f"No hand-written FP8 GEMM R4 for {sm} at {so_path}; run "
            "scripts/build_fp8_hand_gemm.py"
        )
    lib = ctypes.CDLL(str(so_path), mode=ctypes.RTLD_LOCAL)
    lib.fp8_hand_r4_gemm_run.restype = ctypes.c_int
    lib.fp8_hand_r4_gemm_run.argtypes = [
...
def is_available() -> bool:
    if not torch.cuda.is_available():
        return False
    sm = _sm_tag()
    return (_LIB_DIR / f"fp8_hand_r4_{sm}.so").exists()


def fp8_gemm_hand_r4(
    A: torch.Tensor,
    B_nk: torch.Tensor,
    ...
    """Per-MmaTile FP8 GEMM, R4 (multi-lane TMA). See module docstring."""
    ...
    if rc != 0:
        raise RuntimeError(f"fp8_hand_r4_gemm_run failed (rc={rc})")
    return out