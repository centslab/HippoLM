...
// Round 1: deeper pipeline + more consumer warps vs Round 0.
//
// Bottleneck analysis from Round 0:
//   - 1.93ms (56 TF) at 4096³ vs CUTLASS 1.42ms (90 TF), 36% slower
//   - Per-block tile: 69μs (R0) vs 51μs (CUTLASS)
//   - 5060 Ti hw peak: 188 TF → 22μs/MmaTile. R0 = 32% peak, CUTLASS = 43% peak
// Hypotheses for the 36% gap:
//   1. NUM_STAGES=2 too shallow (CUTLASS uses 3)
//   2. Only 4 consumer warps for 128x128 = small per-warp sub-tile
//      (CUTLASS 87a uses 8 consumer warps for same MmaTile)
//
// Round 1 changes:
//   - NUM_STAGES: 2 → 3
//   - Consumer warps: 4 (2x2 grid, 64x64 each) → 8 (2x4 grid, 64x32 each)
//   - Total: 12 warps (1 producer WG of 4 + 2 consumer WGs of 4 each)
//   - Smem: 3 stages × 32 KiB = 96 KiB (under 99 KB cap)
//
// Same MMA, ldmatrix, scale, store as R0.
...
// =====================================================================
// TMA + mbarrier + smem swizzle (same as R0)
// =====================================================================
...
constexpr int NUM_STAGES = 3;  // back to 3

constexpr int WARPS_PER_WG = 4;
constexpr int THREADS_PER_WARP = 32;

// 1 producer WG + 2 consumer WGs = 3 WGs × 4 warps = 12 warps = 384 threads
constexpr int NUM_PRODUCER_WG = 1;
constexpr int NUM_CONSUMER_WG = 2;
constexpr int NUM_TOTAL_WG = NUM_PRODUCER_WG + NUM_CONSUMER_WG;
constexpr int TOTAL_WARPS = NUM_TOTAL_WG * WARPS_PER_WG;
constexpr int THREADS_PER_BLOCK = TOTAL_WARPS * THREADS_PER_WARP;  // 384

constexpr int NUM_CONSUMER_WARPS = NUM_CONSUMER_WG * WARPS_PER_WG;  // 8
constexpr int CONSUMER_THREADS = NUM_CONSUMER_WARPS * THREADS_PER_WARP;  // 256

// 2x4 grid (8 consumer warps), WARP_M=64, WARP_N=32 → 64*32 per warp
...
fp8_gemm_hand_r1_kernel(
    int M, int N, int K,
    int num_tiles_m, int num_tiles_n,
    const float *__restrict__ SFA,  // [num_tiles_m, K/128] CUTLASS MN-major
    ...
    bf16 *__restrict__ D)
{
    ...
    if (tid == 0) { printf("R1: init done, wg_id=%d warp_in_wg=%d\n", wg_id, warp_in_wg); }

    // ── Producer WG (wg_id == 0): lane 0 of warp 0 issues TMA ──
    if (wg_id == 0) {
        if (tid == 0) { printf("R1: producer entry, wg_id=%d warp_id=%d lane_id=%d\n", wg_id, warp_id, lane_id); }
    ...
    {
        const int cwg_id = wg_id - 1;     // 0 or 1
        const int consumer_warp = cwg_id * WARPS_PER_WG + warp_in_wg;  // 0-7
        const int warp_row = consumer_warp / WARPS_N;       // 0 or 1
        const int warp_col = consumer_warp % WARPS_N;       // 0-3
        const int m_warp_base = warp_row * WARP_M;
        const int n_warp_base = warp_col * WARP_N;
        if (lane_id == 0) { printf("R1: consumer warp_id=%d cwg_id=%d cw=%d wr=%d wc=%d\n", warp_id, cwg_id, consumer_warp, warp_row, warp_col); }
        ...
        for (int tile_id = blockIdx.x; tile_id < total_tiles; tile_id += num_blocks) {
            ...
            for (int k = 0; k < num_k_tiles; k++) {
                ...
                // Per-K-tile scale FMA into accum
                ...
                // Sync all consumer warps, then 1 thread per warp signals empty
...
// =====================================================================
// Public ABI: per-MmaTile FP32 scales, CUTLASS MN-major layout
// =====================================================================

extern "C" int fp8_hand_r1_gemm_run(
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    ...
    if (M % 128 != 0 || N % 128 != 0 || K % 128 != 0) {
        fprintf(stderr, "fp8_hand_r1: M=%d N=%d K=%d not all divisible by 128\n",
                M, N, K);
        return -1;
    }
    ...
    CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &occ, fp8_gemm_hand_r1_kernel, THREADS_PER_BLOCK, SMEM_SIZE));
    int num_blocks = std::min(num_sm * occ, total_tiles);

    dim3 grid(num_blocks);
    dim3 block(THREADS_PER_BLOCK);

    fp8_gemm_hand_r1_kernel<<<grid, block, SMEM_SIZE, stream>>>(
