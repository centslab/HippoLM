...
// Round 2: R0 + 8 consumer warps (no other changes).
//
// Bottleneck analysis from Round 1 isolation (NUM_STAGES=3 alone is bad
// because consumer is faster than producer). The real lever is more
// consumer warps for better SM utilization.
//
// Round 2 changes vs Round 0:
//   - Consumer warps: 4 (2x2, 64x64 each) -> 8 (2x4, 64x32 each)
//   - WARP_M=64, WARP_N=32, MMA_M=4, MMA_N=4
//   - Total warps: 9 (1 producer + 8 consumer)
//   - Total threads: 288
//   - NUM_STAGES=2 (same as R0 — R1-min showed NUM_STAGES=3 hurts here)
//
...
// =====================================================================
// Round 2 kernel — R0 + 8 consumer warps (no other changes).
// Producer warp 0 lane 0 issues TMA; consumer warps 1-8 do MMA in a
// 2x4 grid (each consumer covers 64x32 of the MmaTile).
// NUM_STAGES=2 kept from R0 (R1-min isolation showed NUM_STAGES=3
...
constexpr int NUM_STAGES = 2;  // keep R0's NS=2 (R1-min showed NS=3 alone is a regression here)
    ...
    bf16 *__restrict__ D)
{
    ...
    if (warp_id == 0 && lane_id == 0) {
        ...
        printf("R2: producer done, total_k=%d\n", total_k);
    }
...
extern "C" int fp8_hand_r2_gemm_run(
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    ...
    if (M % 128 != 0 || N % 128 != 0 || K % 128 != 0) {
        fprintf(stderr, "fp8_hand_r2: M=%d N=%d K=%d not all divisible by 128\n",
                M, N, K);
        return -1;
    }
    ...
    CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &occ, fp8_gemm_hand_r2_kernel, THREADS_PER_BLOCK, SMEM_SIZE));
    int num_blocks = std::min(num_sm * occ, total_tiles);

    dim3 grid(num_blocks);
    dim3 block(THREADS_PER_BLOCK);

    fp8_gemm_hand_r2_kernel<<<grid, block, SMEM_SIZE, stream>>>(
