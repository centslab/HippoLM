...
// Round 4: R2 + multi-lane TMA (parallel A+B issuance).
//
// Bottleneck analysis from R2/R3: 1 lane issuing TMA limits producer
// throughput. R2 (8 consumers) and R3 (warp specialization + NS=3)
// both stuck at 60-72 TF because the single-lane TMA producer is the
// bottleneck. CUTLASS 87a (89 TF) uses the same single-lane TMA but
// with sophisticated scheduling — hard to replicate by hand.
//
// Round 4 breaks the producer bottleneck by issuing TMA from 2 lanes
// in parallel: lane 0 issues TMA_A, lane 1 issues TMA_B simultaneously.
// TMA unit instruction-issue is parallel; HBM throughput is shared but
// instruction throughput doubles. Expected: 10-20% gain over R2.
//
// Round 4 changes vs Round 2:
//   - Producer: lane 0 issues TMA_A, lane 1 issues TMA_B in parallel
//     (was: lane 0 issues both sequentially)
//   - Same 8 consumer warps (2x4 grid, 64x32 each)
//   - Same NUM_STAGES=2 (R1-min showed NS=3 hurts here)
//   - 9 warps total (1 producer + 8 consumer), 288 threads/block
//
...
// (`data[m_tile + k_tile * (M/128)]`). Round 4 accepts the natural
...
// =====================================================================
// Round 4 kernel — R2 + multi-lane TMA (lane 0 issues A, lane 1 issues B).
// Producer warp 0 lanes 0+1 issue TMA in parallel; consumer warps 1-8
// do MMA in a 2x4 grid (each consumer covers 64x32 of the MmaTile).
// NUM_STAGES=2 kept from R2 (R1-min isolation showed NUM_STAGES=3
// alone is BAD: consumer is faster than producer, deeper pipeline
// just adds initial-fill latency without throughput gain).
// =====================================================================
...
constexpr int NUM_STAGES = 2;  // keep R2's NS=2 (R1-min showed NS=3 alone is a regression here)

constexpr int WARPS_PER_BLOCK = 9;       // 1 producer warp + 8 consumer warps
constexpr int THREADS_PER_WARP = 32;
constexpr int THREADS_PER_BLOCK = WARPS_PER_BLOCK * THREADS_PER_WARP;  // 288

constexpr int NUM_CONSUMER_WARPS = 8;     // warps 1-8
constexpr int CONSUMER_THREADS = NUM_CONSUMER_WARPS * THREADS_PER_WARP;  // 256

constexpr int WARP_M = 64;        // each consumer warp covers 64 rows
constexpr int WARP_N = 32;        // each consumer warp covers 32 cols (split 128 cols across 4 warps)
constexpr int WARPS_M = BM / WARP_M;  // 2
constexpr int WARPS_N = BN / WARP_N;  // 4

constexpr int MMA_M = WARP_M / 16;  // 4
constexpr int MMA_N = WARP_N / 8;   // 4 (2x4 grid of 64x32 sub-tiles)
    ...
    bf16 *__restrict__ D)
{
    ...
    if (tid == 0) { printf("R4: init done\n"); }

    // ── Lanes 0+1 of warp 0: issue TMA loads in parallel ──────────
    //   Lane 0 issues TMA_A (and the per-stage mbarrier_expect_tx)
    //   Lane 1 issues TMA_B
    // Both lanes do the empty_barrier wait so neither races ahead.
    // Same warp → SIMT lockstep → both stages stay in sync.
    if (warp_id == 0 && (lane_id == 0 || lane_id == 1)) {
        int stage = 0;
        int phase = 0;
        int total_k = 0;
        ...
        if (lane_id == 0) {
            printf("R4: producer done, total_k=%d\n", total_k);
        }
    }

    // ── Consumer warps (warps 1-8): compute (MMA + per-K-tile scale + accumulate) ──
    if (warp_id >= 1 && warp_id < 1 + NUM_CONSUMER_WARPS) {
        // 2x4 grid: warp_id 1-4 = warp_row=0; warp_id 5-8 = warp_row=1
    ...
    // R4: removed cleanup (mbarrier_inval after main loop crashes
    // for >48KB dynamic smem on sm_120 — same as R1-min. Next
    // launch re-inits barriers; not needed for correctness.)
}
...
extern "C" int fp8_hand_r4_gemm_run(
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    ...
    if (M % 128 != 0 || N % 128 != 0 || K % 128 != 0) {
        fprintf(stderr, "fp8_hand_r4: M=%d N=%d K=%d not all divisible by 128\n",
                M, N, K);
        return -1;
    }
    ...
    CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &occ, fp8_gemm_hand_r4_kernel, THREADS_PER_BLOCK, SMEM_SIZE));
    int num_blocks = std::min(num_sm * occ, total_tiles);

    dim3 grid(num_blocks);
    dim3 block(THREADS_PER_BLOCK);

    fp8_gemm_hand_r4_kernel<<<grid, block, SMEM_SIZE, stream>>>(
