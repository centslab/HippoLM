...
constexpr int NUM_STAGES = 4;  // ← ONLY change vs R0
...
extern "C" int fp8_hand_r1min_n4_gemm_run(
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    ...
    CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &occ, fp8_gemm_hand_r1min_n4_kernel, THREADS_PER_BLOCK, SMEM_SIZE));
    int num_blocks = std::min(num_sm * occ, total_tiles);

    dim3 grid(num_blocks);
    dim3 block(THREADS_PER_BLOCK);

    fp8_gemm_hand_r1min_n4_kernel<<<grid, block, SMEM_SIZE, stream>>>(
