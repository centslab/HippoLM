...
constexpr int NUM_STAGES = 3;  // ← ONLY change vs R0
    ...
    bf16 *__restrict__ D)
{
    ...
    if (warp_id == 0 && lane_id == 0) {
        ...
        int phase = 0;
        int total_k = 0;
        printf("R1m: PROD ENTER blockIdx.x=%d\n", blockIdx.x);
        for (int tile_id = blockIdx.x; tile_id < total_tiles; tile_id += num_blocks) {
            ...
            for (int k = 0; k < num_k_tiles; k++) {
                if (total_k >= NUM_STAGES) {
                    printf("R1m: PROD block=%d WAIT empty[%d] phase=%d (tk=%d)\n", blockIdx.x, stage, phase^1, total_k);
                    mbarrier_wait(smem_u32(&smem.empty_barrier[stage]), phase ^ 1);
                    printf("R1m: PROD block=%d WAIT empty[%d] OK\n", blockIdx.x, stage);
                }
                ...
                tma_B.load_2d(k * BK, bn * BN,
                              ...
                              smem_u32(&smem.full_barrier[stage]));
                printf("R1m: PROD block=%d k=%d stage=%d phase=%d done\n", blockIdx.x, k, stage, phase);
                stage++;
                if (stage == NUM_STAGES) { stage = 0; phase ^= 1; }
                total_k++;
            }
        }
        printf("R1m: PROD block=%d done total_k=%d\n", blockIdx.x, total_k);
    }
    ...
    if (warp_id >= 1 && warp_id < 1 + NUM_CONSUMER_WARPS) {
        ...
        for (int tile_id = blockIdx.x; tile_id < total_tiles; tile_id += num_blocks) {
            ...
            for (int k = 0; k < num_k_tiles; k++) {
                if (blockIdx.x == 0 && warp_id == 1) printf("R1m: CONS block=%d warp=%d BEFORE wait k=%d stage=%d phase=%d\n", blockIdx.x, warp_id, k, stage, phase);
                mbarrier_wait(smem_u32(&smem.full_barrier[stage]), phase);
                if (blockIdx.x == 0 && warp_id == 1) printf("R1m: CONS block=%d warp=%d AFTER wait k=%d stage=%d\n", blockIdx.x, warp_id, k, stage);
    ...
    // R1m: removed cleanup to test if it's the crash point
    // __syncthreads();
    // if (tid == 0) {
    //     for (int s = 0; s < NUM_STAGES; s++) {
...
extern "C" int fp8_hand_r1min_gemm_run(
    int M, int N, int K,
    ...
    cudaStream_t stream)
{
    ...
    CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &occ, fp8_gemm_hand_r1min_kernel, THREADS_PER_BLOCK, SMEM_SIZE));
    int num_blocks = std::min(num_sm * occ, total_tiles);

    dim3 grid(num_blocks);
    dim3 block(THREADS_PER_BLOCK);

    fp8_gemm_hand_r1min_kernel<<<grid, block, SMEM_SIZE, stream>>>(
